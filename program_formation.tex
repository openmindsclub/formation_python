...
moyenne = {"Khaled": 12, "Ahmed": 10, "Mustapha": 17}
        ...
        \questionbox{C'est quoi un dictionnaire ?}

        Les dictionnaires sont des objets qui contiennent d'autres objets (comme les listes), mais 
        la différence avec les listes, c'est que l'accès aux éléments du dictionnaires se fait par clé,
        et non par indice.

        Revenons au dictionnaire \code{moyenne} que nous venons de crée. Pour accéder à la moyenne 
        de Khaled, il suffit de faire ça:
            \begin{lstlisting}[style=code]
# Création du dictionnaire
moyenne = {"Khaled": 12, "Ahmed": 10, "Mustapha": 17}

# Ceci nous affichera 12
print(moyenne["Khaled"])
            \end{lstlisting}       

            Dans l'exemple précédent, nous avons directement affiché la moyenne de Khaled, mais on
            aurait pu la stocker dans une variable, et faire ce qu'on veut avec.
            \begin{lstlisting}[style=code]
moyenne_khaled = moyenne["Khaled"]

# Ceci nous affichera 12
print(moyenne_khaled)
            \end{lstlisting}       

            Pour afficher un dictionnaire en entier, il suffit juste de passer le dictionnaire à
            \code{print} :
             \begin{lstlisting}[style=code]
moyenne = {"Khaled": 12, "Ahmed": 10, "Mustapha": 17}
...
print(moyenne)
        ...
        \subsubsection{Modifier la valeur d'un élément du dictionnaire}    
        Disons que nous avons fait une erreur en calculant la moyenne de Khaled, pour modifier 
        sa moyenne après la création du dictionnaire, il suffit de faire ça :
            \begin{lstlisting}[style=code]
moyenne = {"Khaled": 12, "Ahmed": 10, "Mustapha": 17}

# On modifie la moyenne de Khaled
moyenne["Khaled"] = 13

# Ceci nous affichera 13
print(moyenne["Khaled"])
            \end{lstlisting}       
...
if "Ahmed" in moyenne:
    ...
    print("Ahmed n'existe pas")
        ...
        Réaliser les tâches suivantes dans un programme.
        \begin{itemize}
            \item Crée un dictionnaire de joueurs et de leurs score. Le dictionnaire contient :
            \begin{itemize}
                \item Mohamed, score 500
                \item Ahmed, score 20
                \item Fawzi, score 0
            \end{itemize}

            \item Ajouter un autre joueur nommé Hamid avec le score 100.

            \item Supprimer le joueur Fawzi du dictionnaire.

            \item Parcourir le dictionnaire et afficher les scores de tous les joueurs.
        \end{itemize}


    \subsection{Les fichiers}
        \questionbox{Pourquoi utiliser les fichiers ?}

        Imaginons que nous voulons crée un jeu vidéos, et dans ce jeu, on veut sauvegarder le score
        des meilleurs joueurs. Avec les variables c'est impossible, car dés que notre programme s'arrête,
        tous les variables disparaissent.

        Mais avec les fichiers, c'est différent ; car le contenu des fichiers reste même après l'arrêt 
        du programme.

        \subsubsection{Ouvrir un fichier}
        Avant de pouvoir lire ou écrire dans un fichier, il faut d'abord l'ouvrir en utilisant 
        la fonction \code{open}. Voici comment on utilise cette fonction :
        \begin{lstlisting}[style=code]
fichier = open("chemin du fichier à ouvrir", "mode d'ouverture du fichier")
        \end{lstlisting}
