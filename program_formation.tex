...
print(x)
        ...
        \subsubsection{Plusieurs paramètres}
...
presentation("Yanis", "Khelloufi", 22)
        ...
        \subsubsection{Fonction qui renvoie plusieurs valeurs}
            On peut aussi crée une fonction que renvoie plusieurs valeurs. Voici un exemple d'une fonction qui fait
...
%             Détaillons un peu de quoi il est composé ce schéma :
%             \begin{itemize}
%                 \item \code{parametre1}, \code{parametre2} sont des variables qu'on donne 
%             \end{itemize}

            % En faite, vous avez déjà utilisé plusieurs fonctions déjà disponible en python. Ces fonctions sont :
            % \code{print()}, \code{input()}, \code{int()}. Dans ces fonctions, il y a bien des instructions qui
            % permettent à ces fonctions de réaliser une tâche bien précise. La tâche de \code{print()} est d'afficher 
            % un message à l'écran, la tâche de \code{input()} est de récupérer la saisie de l'utilisateur \ldots
        
%             Donc quelques parts dans votre système, il y a un fichier ou la fonction \code{print()} est crée comme 
%             ça :
%             \begin{lstlisting}
% def print(paramatres) :
